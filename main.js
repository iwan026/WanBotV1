...
zeroyt7.on("group-participants-update", async (anu) => {
    ...
    try {
      ...
      if (anu.action == "add" && !mem.includes(zeroyt7.user.jid)) {
        ...
        wel = `Hallo @${anu_user}\nSelamat Datang Di Group ${mdata.subject}\n\n*ADMIN MELAYANI*\nãƒ¡ _*TOP UP GAME*_\nãƒ¡ _*ISI ULANG PULSA*_\nãƒ¡ _*TOP UP SALDO EWALLET*_ \nãƒ¡ _*TOKEN LISTRIK*_ \nãƒ¡ _*JASA POST (FREE)*_ \nãƒ¡ _*JASA REKBER*_ \n\nãƒ¡ _*LIST HARGA? LANGSUNG PC AJA*_ \nSemoga Betah*_`
      ...
      if (anu.action == "remove" && !mem.includes(zeroyt7.user.jid)) {
        ...
        time_wel = moment.tz("Asia/Jakarta").format("HH:mm")
        memeg = mdata.participants.length
        out = `Selamat Tinggal Kak @${anu_user} Jangan Balik Lagi Yaa`
        buff = await getBuffer(
